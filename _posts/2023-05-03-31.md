...
조건의 결과가 true, false로 구분(""사이 공백 조심)
...
- line8 : 앞에 n 또는 N이 있으면 모든 단어를 인정
- line12: 정상적인 종료가 아니므로 exit 1으로 종료
...
and = -a 또는 &&, or = -o 또는 \\(파이프기호 두개) -a 나 –o 는 테스트문 안에서 사용될 수 있는데 괄호는 특수문자 앞에는 역슬레쉬(\)를 붙여줘야 한다.

- If [ \( -f \ $fname \) –a \( -s \ $fname \) ] ; then
( -f = 일반파일, -s = 크기가 0이 아닌 파일
...
1.for문 설명 : for 변수 in 값1 값2 값3 ...<br>
do<br>
반복할 문장<br>
done
...
- 현재 디렉토리 위치에서 *.sh로 끝나는 셀 스크립트 파일들의 파일명과 파일의 내용을 앞에서 3줄씩 화면에 표시하는 스크립트를 작성하세요
...
조건식이 참인 동안 계속 반복
- 조건식에 [1] 또는 [:] 또는 while [true]가 오면 항상 참이다.
- 무한 루프 벗어나기 : Ctrl + C
...
예제 : 비밀번호를 입력 받고 비밀번호가 맞을 때 까지 계속 비밀번호를 입력받는 스크립트를 작성하세요
...
조건이 참이 될 때까지 반복 - 거짓인 동안 만 반복
- 예제 - 1에서 10까지 합을 구하는 스크립트 until 문으로 작성하세요
...
### break, continue, exit 문

- break – 조건문 빠져 나갈 때
- continue - 조건식으로 들어갈 때
- exit - 프로그램 완전 종료시 사용
...
외부변수(전역변수)로 선언하며 다른 프로그램에서도 변수로 사용
- exp1.sh line2~3 : var1, var2 변수출력, var2는 exp2.sh에서 외부변수로 선언
- exp2.sh line 2 : var1에 값을 넣는다. 일반변수로 exp2.sh내에서 만 사용된다.
...
- printf “%5.2f \n\n \t %s \n
- %5.2f = 실수로 5자리 소수점 2째 자리 출력, \n = 줄바꿈, \t =Tab, %s = string(문자열) 출력
- 문자열 출력시 “$var2” - 중간에 공백이 있으므로 “ “ 로 묶어줘야한다.
<br>
...
리눅스 명령어를 결과로 사용하기 위해서는 $(명령어) 사용해야 한다.
- 결과를 파라미터로 사용하고자 할 때는 set명령어를 같이 사용한다.(일,월,화,수,목,금,토의 순서)
- 2022. 05. 04. (수) 11:20:10
- $1    $2   $3  $4     $5   $
